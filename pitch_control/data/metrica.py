...
def fetch_data(dataset_name, **kwargs):
    ...
    if dataset_name == "game1_tracking":
        ...
        available_datasets = ["game1_tracking", "game2_tracking", "game3_tracking",
                              "game3_events"]
        raise ValueError(
...
def calculate_velocity(x, y, time, window=3):
    ...
    for i in range(window, len(x) - window):
        if not (np.isnan(x[i - window:i + window + 1]).any() or np.isnan(
                y[i - window:i + window + 1]).any()):
...
def extract_coordinates(dataset, include_velocity=True, include_acceleration=False,
                        velocity_window=3):
    """
    ...
    if len(df) > 1:
        ...
        frame_rate = 1.0 / time_diff.total_seconds() if hasattr(time_diff,
                                                                'total_seconds') else 1.0 / float(
            time_diff)
        duration = df['timestamp'].max() - df['timestamp'].min()
        duration = duration.total_seconds() if hasattr(duration,
                                                       'total_seconds') else float(
            duration)
    else:
        frame_rate = 25.0
        duration = 0
    ...
    if 'ball_x' in df.columns and 'ball_y' in df.columns:
        ...
        if include_velocity:
            ...
            if include_acceleration:
                ax, ay, accel = calculate_acceleration(vx, vy, df['timestamp'].values,
                                                       velocity_window)
                ball_data['ax'] = ax
                ball_data['ay'] = ay
    ...
    for x_col in player_columns:
        ...
        team = 'home' if 'Home' in player_id or any(
            h in player_id.lower() for h in ['home', 'h_']) else 'away'
...
if __name__ == "__main__":
    ...
    coords = extract_coordinates(dataset, include_velocity=True,
