...
def create_sample_players():
    ...
    home_velocities = [
        (0, 0),  # GK stationary
        (2, 1), (2, -1),  # CBs moving up
        (3, 2), (3, -2),  # FBs pushing forward
        (1, 3), (1, -3),  # CMs spreading
        (4, 1), (4, -1),  # Wings running
        (2, 2), (3, -1)  # Forwards
    ]

    positions_types = [Position.GK, Position.CB, Position.CB, Position.FB, Position.FB,
                       Position.CM, Position.CM, Position.WM, Position.WM, Position.FW,
                       Position.CF]
    ...
    for i, ((x, y), (vx, vy), pos_type) in enumerate(
        ...
        player = Player(f"H{i + 1}", f"Home Player {i + 1}", "Home", pos_type, i + 1,
                        physics)
    ...
    for i, ((x, y), (vx, vy), pos_type) in enumerate(
        ...
        physics = PlayerPhysics.for_position(pos_type)
        player = Player(f"A{i + 1}", f"Away Player {i + 1}", "Away", pos_type, i + 1,
                        physics)
        player_state = player.create_state(Point(x, y), Point(vx, vy))
        players.append(player_state)
...
def main():
    ...
    for player in players[:6]:  # Show vectors for subset to avoid clutter
        ...
        ax.arrow(player.position.x, player.position.y, dx, dy,
