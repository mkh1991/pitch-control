...
def profile_backend(backend: str, players: List = None, ball_position: Point = None,
                    grid_resolution: tuple = (84, 54), n_runs: int = 3):
    """
    ...
    if use_numba:
        ...
        from pitch_control.models.spearman import _calculate_times_vectorized, \
            _calculate_ball_travel_times, _calculate_control_probabilities
        _ = _calculate_times_vectorized(positions, velocities, grid_points, max_speeds,
                                        accelerations, reaction_times)
        _ = _calculate_ball_travel_times(ball_pos_array, grid_points,
                                         model.spearman_config.average_ball_speed)
        adjusted_times = np.random.random((len(players), len(grid_points))) * 2.0
        _ = _calculate_control_probabilities(adjusted_times, team_ids,
                                             model.spearman_config.sigma)
...
def compare_backends(n_runs=20):
    ...
    for res in resolutions:
        ...
        # Profile NumPy
        numpy_results = profile_backend("numpy", players, ball_position,
                                        res,
                                        n_runs)

        # Profile Numba
        numba_results = profile_backend("numba", players, ball_position,
                                        res,
                                        n_runs)
        # Print comparison
        print("\nCOMPARISON SUMMARY:")
        ...
        numba_time, numpy_time = numba_results["total_core"], numpy_results["total_core"]
